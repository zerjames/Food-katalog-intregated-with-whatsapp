...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (empty($error)) {
		if (empty($name)) {
			...
			update_product($product_id, $productData);
			$success = 'Produk berhasil diupdate';
			// Auto refresh page to show updated checkboxes
			header("Refresh:0");
			exit;
		}
	}
	...
	<div class="container">
		...
		<div class="card" style="max-width: 600px;">
			...
			<form method="post" enctype="multipart/form-data" id="productForm">
				<label>Nama Produk *
					<input class="input" type="text" name="name" value="<?php echo htmlspecialchars($product['name']); ?>" required>
				</label>
				
				<div class="mt-2"></div>
				<label>Harga Jual *
					<input class="input" type="number" name="price" value="<?php echo $product['price']; ?>" min="1" required>
				</label>
				
				<div class="mt-2"></div>
				<label>Harga Beli
					<input class="input" type="number" name="cost" value="<?php echo $product['cost']; ?>" min="0">
				</label>
				
				<div class="mt-2"></div>
				<label>Stok
					<input class="input" type="number" name="stock" value="<?php echo $product['stock']; ?>" min="0">
				</label>
				...
				<label>Gambar Produk
					...
					<?php if (!empty($product['image'])): ?>
						<div style="margin-top: 8px;">
							<img src="<?php echo $config['uploads_url'] . htmlspecialchars($product['image']); ?>" style="max-width: 200px; max-height: 150px; border-radius: 8px;">
				...
				<div class="mt-2"></div>
				<label>Diskon (%)
					<input class="input" type="number" name="discount_percentage" value="<?php echo htmlspecialchars($product['discount_percentage'] ?? ''); ?>" min="0" max="100">
				</label>

				<div class="mt-2"></div>
				<label class="flex" for="discount_active">
					<input type="checkbox" id="discount_active" name="discount_active" <?php echo isset($product['discount_active']) && $product['discount_active'] ? 'checked' : ''; ?>>
					<span>Aktifkan Diskon</span>
				</label>

				<div class="mt-4"></div>
				<button class="button" type="submit">Update Produk</button>
			</form>
		</div>
		<div class="mt-4">
